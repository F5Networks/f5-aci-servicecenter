...
.. note::
    - A VLAN configuration form is displayed to add Self IPs. For VLAN, the fields to be configured are (1) Partition and (2) Route Domain. For self IPs, the fields to be configured are (1) Address (2) Netmask (3) Traffic-group (4) Port-lockdown.
    - Partitions and Route Domains should be created from BIG-IP UI
    - If the Partition and Route Domain is not selected during creation of VLAN/Self IP, it will be assumed to be Common Partition and Route Domain 0 by default.
...
F5OS
----------------------------------------
Features available on F5OS:
- Visibility: Only VLAN table will be available;VIP table and Node table will not be available. 
- L2-L3 Network Management: VLAN configuration will be available. Self IP and Default Gateway configuration will be disabled. All the VLAN workflows will be the same as mentioned in VLAN workflows.
- L4-L7 App Services tab will be disabled and unavailable
...
.. note::
    - A VLAN configuration form is displayed to add VLANs and information for one or more self IPs. For VLAN, the fields to be configured are: (1) Name (2) Tag (3) Interface.
    - If a configuration with 2 Tenants on separate rSeries/VELOS hosts in HA configuration has to be supported, the 2 rSeries/VELOS hosts will need to have the same VLAN name. Hence please add the same VLAN Name for such rSeries/VELOS Hosts - guests of which hosts are going to form HA pair. The steps to do so are as follows:
    
      - Create a VLAN on rSeries/VELOS host 1
      
      - Copy the VLAN name from host 1 and create VLAN on host 2 with same name
      
      - Assign these VLANs to the corresponding Tenants Tenant 1 (on host 1) and Tenant 2 (on host 2)
...
L2-L3 Network Management workflows for Tenants will be different since VLAN configuration will be unavailable. Only Self IP configuration and Default Gateway configuration will be available. Default configuration workflows will be same as mentioned in the VLAN workflows. SelfIP workflows will be as below.
...
**Q. Why do I see duplicate VLAN entries with the same VLAN name and different VLAN tag when the L2-L3 Stitching Tab is loaded on the vCMP Guest/Tenant device?**

A VLAN is created using the L2-L3 Network Management Tab on the vCMP/F5OS Host device. This VLAN can be assigned to a vCMP Guest/Tenant device present on its Host. Here, both the vCMP/F5OS  Host and the vCMP Guest/Tenant share the same name and tag of the VLAN. When the tag of this VLAN is modified from the vCMP/F5OS Host device, the new VLAN tag gets updated on the Host however vCMP Guest/Tenant device creates a duplicate VLAN configuration with the new tag. This is an expected behaviour from the BIG-IP. To properly update the VLAN tag on a vCMP/F5OS device, it is recommended to follow the below steps:

1. Within the vCMP Guest/Tenant, delete the relevant VLAN from within the Guest/Tenant.
2. On the vCMP/F5OS Host, remove the VLAN publication from the Guest/Tenant.
3. On the Host, modify the tag of the host-based VLAN.
4. On the Host, publish the VLAN to the Guest/Tenant.
5. Within the Guest/Tenant, view the VLAN from within the Guest/Tenant.
