...
This is a known issue for BIG-IP v 12.x. If a pool with nodes is associated with a self IP of the same subnet, the BIG-IP doesn’t allow user to delete that self IP. As a result, the VLAN delete operation also fails with the error.
...
**Q. why do I see “VLAN not available for stitching”, and I am unable to configure the VLAN ?**
