...
The default gateway is created on the BIG-IP device and is added to the application database. For an HA setup, the default gateway will automatically sync to the BIG-IP peer device. F5 ACI ServiceCenter has added support for brownfield default gateway from v2.9 where default gateway will automatically sync to FASC database once user navigates to L2-L3 tab. FASC has added support to allow any Default Gateway name to be synced to FASC database (Until version 2.8, the only default gateway name supported for  was **apic-default-gateway**). 
