...
Dynamic Endpoint attach/detach using the Manage mappings button is not supported for BIG-IP devices which are in HA pair. If used, the behavior is unknown and user may experience BIG-IP service restarts and hang up.

**Workaround**: None

-------

**Restrictions on dynamic mapping update and delete**

When a dynamic endpoint mapping is added to application using the “Manage Endpoint Mappings“, the application gets created on BIG-IP. Any update or delete of this endpoint mapping is not supported. And if update or delete is attempted, it will retain the original dynamic mapping values.

**Workaround**: Delete the AS3 application and recreate it with the updated dynamic endpoint mapping. Please note that there will be disruption of traffic due to this delete and recreate operation.

-------

**AS3 application can either have static nodes or dynamic nodes but not both**

AS3 application will support either static nodes or dynamic nodes (Using dynamic endpoint mapping) but not both. 
...
**Workaround**: Ensure that the APIC endpoint subnet/IPs is different from the static endpoints on the BIG-IP.

-------

**Same Dynamic endpoint mappings on 2 separate partitions of a BIG-IP is not supported**

If 2 applications belonging to different partitions are associated with the same APIC Endpoint Group (Tenant|Application|EPG), the dynamic discovery of nodes will not work for either of the AS3 applications. 

**Workaround**: Remove erroneous applications and recreate with supported mappings 

-------

**BIG-IP hangs if more than 70 endpoints are dynamically discovered in APIC EPG**

If an APIC endpoint group has more than 70 endpoints attached, then the endpoint list will not get reflected on the BIG-IP, and users may experience that services restart on BIG-IP. 

**Workaround**: For any dynamic endpoint mapping, please ensure that the number of endpoints in the corresponding APIC endpoint group never cross 70.
