...
**When I try to create a new partition to deploy a new application with Service Discovery, why don’t I find the options to map the APIC endpoints to a Route Domain that resides in a non-common partition?**
...
**Why do I receive an AS3 error “TMSH CLI script failed during execution” when trying to delete a partition or an application with a Route Domain that resides in a non-common partition?**
