...
6. Optionally, click on :guilabel:`Manage Endpoint Mappings` to configure Dynamic Endpoint Mappings for endpoint discovery, which dynamically updates endpoints in BIG-IP pools whenever endpoints get attached or detached from APIC Endpoint Groups.
...
.. note::
   ...
   - :guilabel:`Manage Endpoint Mappings` APIC VRF field is read-only and is retrieved from the APIC based on the provided info.
...
5. Optionally, click :guilabel:`Manage Endpoint Mappings` to configure Dynamic Endpoint Mappings for endpoint discovery, which dynamically updates endpoints in BIG-IP pools whenever endpoints get attached or detached from APIC Endpoint Groups.
...
.. note::
   ...
   -  :guilabel:`Service-discovery/http template` provides endpoint discovery, which dynamically updates endpoints in BIG-IP pools whenever endpoints get attached or detached from APIC Endpoint Groups.
...
2. This will display a list of all the AS3 applications present on the BIG-IP device. The list will have the following columns:
   ...
   **Pool**    
   ...
   - BIG-IP pool names which are present in this application. Each pool should be displayed on a separate row. The pool has a hyperlink to BIG-IP UI to the same pool.
...
Refresh L4-L7 App Services tab
-------------------------------
